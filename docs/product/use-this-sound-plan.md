...
| **B — editor attach + mixdown (Android shipped 2026-09-08; iOS pending)** | Sound tool "♪ Pick sound from a video…" → passthrough extraction (`MemeVideoSound`: extractor→muxer m4a ≤ 60 s + MediaCodec PCM decode) → attach/replace/remove + volume; dual-player preview glued to the timeline clock; export mixes cues + soundtrack into ONE PCM bed (`MemeSoundMix`, common-tested) riding the existing second Transformer sequence; m4a persists with the slot (asset `sound`) and rehydrates on resume (undecodable → row stripped + named notice, never a silent export); publish uploads the m4a hash-verified BEFORE signing, verifies sha vs the wire, then stamps sound/p/attribution with the real URL | attach → caption → export round-trip on device; common bed tests + state tests green; nothing stamps pre-upload |
| **C — "Use this sound" from a bitz** | bitz ⋯ menu / player / remix surface action → download + extract + seed editor with the soundtrack; feed chip on published notes ("♪ sound of …" → source) | end-to-end on both platforms; provenance survives round-trip (fixture) |
