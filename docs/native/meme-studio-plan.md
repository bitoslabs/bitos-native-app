...
| MST-054 | GIF-sticker picking in the video editor REUSES the existing APP-008 system end-to-end (user review catch — no duplicate picker): the shared `GifPickerContract` (endpoints, preview-fallback parse, recents merge, versioned 24 h cache wire, beta key) and the composer's `GifPickerSheet` on BOTH platforms (recents/trending/search/debounce/paging already inside). The meme integration only adds the bridge into the MST-053 animated-layer path: pick → full-source download (OkHttp / URLSession, bounded ≤24 MiB before bytes reach the editor) → cache file / asset-with-data → the SAME layer insert → reel decode → animated on stage + looping in export. Android entry: layers sheet "Add animated GIF sticker…"; iOS entry: the video-mode layer button. An earlier draft duplicated the contract/sheet/cache as `GiphyCatalog`/`GifStickerPicker*`/`GiphyClient` — deleted wholesale on review. | both | both apps compile + suites green against the pre-existing sheet; QA: open the meme GIF picker → identical behavior to the composer picker (recents persist, trending caches 24 h) → pick → animated layer |
