...
| MST-036                            | Export quality/size controls: resolution (1080p/720p/480p) × quality (High/Med/Low) preset picker with **Auto default = pre-MST-036 behavior** (1080p, highest tier, publish cut-ladder untouched). Shared `MemeExportPresets` single-sources the bitrate table (6/4/2.5, 4/2.5/1.5, 2/1.2/0.8 Mbps), audio 128 kbps, the MB estimate `(video+audio) × trimmed duration + 3% container` (integer math, byte-identical both platforms), the deterministic `≈ N.N MB` label, and the manual publish gate: estimate ≤ 57.6 MiB (Blossom 64 MiB − 10% ABR-overshoot headroom); manual picks over the gate BLOCK Publish ("too large, pick a lower preset") — never silently degraded, the ladder only owns Auto. Actual MB always shown post-export (estimate vs. actual when it fails). fps stays fixed at 30 (not exposed); GIF export excluded (own 8 MB ladder). Preset choice resets to Auto per project (session-only persistence).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | both     | shared: `MemeExportPresetsTest` green **SHIPPED**; Android: `MemeVideoRender` pins `DefaultEncoderFactory` video/audio bitrates (never encoder defaults) + preset `ScaleAndRotateTransformation` before grade/overlay with paint plans on the output canvas + exporters return actual export dims so publish imeta carries the REAL canvas **SHIPPED**; iOS: `memeEncoderPlan`/`memeExportEstimate` bridge seams (common + Swift contract tests; no Swift table mirror) + plan-driven canvas + generic reader→writer bitrate-pinning transcode pass (AVAssetExportSession presets expose no bitrate) + actual-dims imeta **SHIPPED** — single-sourced `MemeExportPresets.encoderPlan` consumed by both adapters; UI **SHIPPED**: session-only picker in both export sheets (Auto chip + 1080p/720p/480p × High/Med/Low chips, video only) with the live shared `≈ MB` estimate + gate line; manual picks over the gate BLOCK publish pre-render with a re-pick ask (estimate MB in the message), manual skips the cut ladder entirely, rare ABR overshoot surfaces ACTUAL MB and aborts before anything signs; Save is never capped and its result carries the actual MB. QA matrix additions: manual-gate block, manual overshoot, Auto unchanged, mixed-clip uniform canvas, estimate vs actual on device. |
| MST-051                            | Post-details wave: (a) the MST-036 quality picker + live `≈ MB` estimate ALSO lives on the post-details screen (video section, under Cover) — an over-cap pick is fixable without leaving the flow; (b) **NIP-13 PoW option** on post details, REUSING the shared PowCard rank UI (`PowDifficultySelector` slider + segmented hash bars + `PowBadge`, extracted from PowCard for both platforms — the mining driver itself is NOT reused: memes mine AFTER the hash-verified upload since the imeta must be final, and BEFORE anything signs), through shared `NoteComposer.composeMeme{Video,Picture}NoteWithPow` + `Pow.mineChunk` (20k-attempt cancellable chunks, 5M hard cap, `Dispatchers.Default` / detached task; exhausted window surfaces "try fewer bits", never mines forever). The published id BYTE-MATCHES the mined template (nonce tag appended last; common-tested mine→compose→verify for kind 20/21/22); bridge seams `mineMeme{Video,Picture}Pow` + `powMeme{Video,Picture}EventId/PublishMessage` for iOS. (c) **Draft persistence**: the post-details draft (caption, alt, tags, CW, license, zaps, PoW) is session-scoped and hoisted to the editor (`PostDetailsDraft` Android / `store.postDraft` iOS) — a Cover-Edit round trip (back → stage "Set cover" → post details) never loses typed text.                                                                                                                        | both     | `NoteComposerTest.powMeme*` + `BusinessCoreBridgeTest.memePowSeams*` green (incl. regression pin: the pre-existing `mineTextNotePowWithTags`/`mineCommentPowWithTags` paste bug that emitted literal `${it.nonce}` junk — iOS text/comment PoW mining was silently dead — fixed); both apps compile; device QA: PoW publish lands on relays with nonce tag + difficulty verified, cover-edit round trip keeps caption                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| MST-052                            | Post-details bug-fix wave: **(1) stuck "done — cannot submit"**: Android's meme publish never reset the SHARED `NotePublisher` state — after one finished/failed attempt `result != null` silently dropped every `publishMeme{Video,Picture}Note` call (the machine sat on the old state forever). `MediaPublishViewModel` now `publisher.dismiss()`es at publish start — exactly what iOS's `beginPublish`/`resumePublish` always did. **(2) render-before-upload verified**: every render site (Save, export sheet, publish `exportNow`/`export`) passes the LIVE picker selection (`preset = exportPreset` / `preset: exportPreset`, all call sites pinned) — publish always re-renders at the picked preset BEFORE the hash-verified upload. **(3) dropped video on resume**: slot `File`s were probed through the content-resolver FD path (built for `content://` picker URIs) — flaky right after a cold start, dropping the whole timeline until a full reopen ("resume opens without the video; reopening works"). New `MemeVideoExport.probeFile` probes plain files by DIRECT PATH (`setDataSource(path)`, the proven `captureCoverJpeg` method), the restore is a retryable `restoreVideoClipsFrom(saved)` (skips already-restored ids), and a one-shot delayed `LaunchedEffect` heals transient drops IN-SESSION — the reopen banner only survives genuinely unreadable clips. iOS resume probes via AVURLAsset (reliable; no change). | both     | full Android unit suites green; QA: publish → edit → publish again without leaving the flow; cold-start draft resume shows the timeline immediately; picked preset size matches the uploaded artifact                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| MST-053                            | Animated GIF sticker layers (web `meme/gif.ts` + `gifLayerPainter` parity): a GIF picked as an image layer becomes a DECODED FRAME REEL (shared `GifDecoder`, ≥2 frames = animated) that paints the frame active at the TIMELINE clock and **LOOPS for the whole export** — never clamps past the end (the web "GIF stopped moving" bug). Wire unchanged: GIF-ness rides the asset bytes. Shared `GifLayerRules` (looping frame selection, `totalDurationMs`, `isAnimated`) is pure + common-tested — both platforms select identical frames. Android **SHIPPED**: `GifLayerReel` + `decodeGifLayerReel` (24 MiB bound, RGBA→Bitmap, ≥2 frames); reel cache auto-decodes EVERY image-layer asset (pick and draft resume both land there); stage `OverlayNode` renders the active frame at the playhead (Compose `Image` on the reel bitmap); export `CanvasOverlay.onDraw` resolves per-frame via the shared rule — and still-image layers now decode ONCE per export (the old path re-decoded the source EVERY frame); layers sheet gains "Add animated GIF sticker…" (system picker, strict `image/gif` MIME — the photo picker has no animated-only type).                                                                                                                                                                                                                                                                                       | both     | `GifLayerRulesTest` green; Android compile + unit suites green; iOS **SHIPPED**: `memeGifFrameIndexAt` bridge seam (shared looping rule, no Swift mirror; common-tested incl. loop wrap + junk → -1); ImageIO `GifLayerReel` decode (≥2 frames, sub-2cs → 100 ms heuristic, 24 MiB bound); `MemeEditorAsset.data` retains ORIGINAL GIF bytes — the slot persists them VERBATIM (`asset-<id>.gif`) instead of PNG-freezing the first frame, and resume reloads the data so an animated layer comes back ANIMATED; stage swaps the active frame at the playhead (`gifFrameAt` resolver, GIF overlays force the timed tree); export burns a CAKeyframeAnimation on `contents` — one rendered content per GIF frame (same paint path), keyTimes normalized on the output clock, looping, ≤2400 keyframes bound; transform/opacity fx ride alongside.                                                                                                                                                                                                                                                                                                                                                                                                                                                             | QA: pick a multi-frame GIF in video mode → animates on stage → save draft → reopen → still animated → export loops through a long clip; still image layers unchanged |
| MST-054                            | GIF-sticker picking in the video editor REUSES the existing APP-008 system end-to-end (user review catch — no duplicate picker) — now WITH the web's **GIFs/Stickers kind toggle**: `GifPickerContract.buildUrl(…, stickers)` hits Giphy's transparent-cut-out endpoints (`/v1/stickers/{trending,search}`); the versioned cache wire bumps to **v2** adding a per-kind trending page + the remembered tab (`CachedGifs.stickersTrending/stickersSavedAtMs/stickersKind`; v1 wires still decode with GIF-kind defaults); both native `GifPickerSheet`s gain the segmented GIFs/Stickers pills (web `GifPicker.svelte` parity) with per-kind trending snapshots and tab persistence, plus a `defaultStickers` entry param — the MEME editors open on Stickers (the web's own note: "exactly what meme layers want"), the composer default stays GIFs. Bridge `gifPickerUrl(stickers)`/`gifCacheEncode(v2)` passthrough. The meme integration only bridges into the MST-053 animated-layer path: pick → full-source download (bounded ≤24 MiB) → cache file / asset-with-data → the SAME layer insert → reel decode → animated on stage + looping in export. An earlier draft duplicated the contract/sheet/cache as `GiphyCatalog`/`GifStickerPicker*`/`GiphyClient` — deleted wholesale on review.                                                                                                                                                  | both     | `GifPickerTest` stickers pins green (endpoints, v2 round-trip, v1 backward decode, unknown-version reject); both apps compile; QA: composer picker unchanged (GIFs default), meme picker opens on Stickers, tab + trending caches persist per kind, pick → animated layer                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
...
| _(2026-09-02 done — publish wiring)_                | Remix lineage SHIPPED into both publish sheets: an optional "Remix of" field (note1/event id) + author npub, a `source · … → you` chips row (scr-pub), and on publish the tags ride via the tested `memeRemixTagsFor` seam (compact ≤700 payload + ladder, `p` attribution) — Android passes `remixTagsJson` to both lanes (picture + video); the Kotlin picture composer seams grew a defaulted `extraTagsJson` (bridge test pins the id change + frame tags); iOS `publishMemePictureNote` carries `remixTagsJson` through both composer seams. Chain-dialog browsing + the target-picker "Remix" entry remain V1.x (feed-side rail already renders chains).                                                                                                                                                                                                                                | both     | `memePictureSeamAcceptsRemixExtraTags` green; both apps compile                                            |
| _(2026-09-07 done — M4b UI reach)_                  | Bitz "Remix" now opens the meme EDITOR (web studio parity) on both platforms: `MemeRemixSeed` handoff (media URL, `meme` payload, lineage) → editor downloads the source (HTTPS ≤64 MiB), imports it as clip/background, clones the layout with fresh ids (`MemeRemix.applyTo`); publish rides `memeRemixTagsFor` with relay hints (`RemixRules.relayHints`: source-tag relays + write relays, ≤3), license (remix default CC-BY-4.0) and `remix of <label>` attribution; imeta reaches full web parity (field order, `duration 4.861`, `bitrate`). No-media notes fall back to the seeded composer. Same day: `["client","BitOS"]` rides all bitz/media composers behind the existing `PrivacyPrefs.includeClientTag` (live-read at publish), and a lineage cycle pre-flight refuses a looping remix at tap time (web studio guard parity). Remaining: RemixChainDialog entry from the rail. | both     | shared lane + Android suite green; iOS swift-6 typecheck green; fixture `docs/source/event-remix-app.json` |
...
| _(2026-09-03 — relay fetch live on BOTH platforms)_ | Android + iOS `SharedTemplateStore` + hub "Shared templates" rails (verified kind-30078 frames → bridge summary, newest-wins ≤24, ⚡price chips) → seeded editors via the apply seams; iOS file registered in project.pbxproj.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | both     | suite green; compiles/parse                                                                                |
| _(2026-09-02 — read path done)_                     | `MemeTemplateContract` + tests + seams `memeSharedTemplateSummary`/`memeApplySharedTemplate` shipped (hostile battery: foreign d-tag/schema/version, empty overlays, junk price→free, icon/category fallbacks, hostile overlay coords clamped via the wire parser, converter-crash guards per MEM-004). Remaining for MST-045: relay fetch → hub rail section, publish-your-own write path; MST-046 zap unlock ledger.                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | shared   | battery green                                                                                              |
...
mascot layers. _(2026-09-03 — first scr-suite slice shipped early:
...
windows, zoom/speed tracks, draw, sound-seed — stays sequenced here.)_
