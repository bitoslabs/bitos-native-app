...
- [◐] M4 MST-040..049 — Studio home (resume slots, templates), sound seed + synth SFX pack (APP-021), looks/fx, shared templates + marketplace (MEM-004), remix payload (`com.bitos.bitz.meme` v1), shared sounds, batch queue, opt-in AI suggestions
  - [x] SOUND SHEET FUNCTIONAL FIX (2026-09-10): "Open" taps appeared dead — the trending/browse overlays rendered BEFORE the editor's main layout in the composable, so they drew UNDERNEATH it (Compose: later siblings win). Both overlay blocks moved to the END of MemeEditorScreen (draw on top, full-screen). Plus press-state polish: every new sound row (trending/browse/shared/library/browse-sheet) now clips BEFORE clickable (`.clip(RoundedCornerShape(12.dp)).clickable(…)`) so the pressed ripple is rounded like the card, not square. APK rebuilt.
  - [x] SOUND SHEET DISCOVERABILITY FIX (2026-09-10): the 🔥 Trending and 🎬 Browse-video rows were CONDITIONAL on feed data (hidden on a quiet feed window — the "I don't see the UI" report) — now ALWAYS visible when no soundtrack is attached; their screens own the empty states (trending: "No borrowed sounds yet"; browse: fires its own kinds-[21,22] REQ while open so rows fill live). Debug APK rebuilt for on-device review.
  - [x] BROWSE VIDEO SOUNDS (kind 21/22) IN THE SOUND SHEET (2026-09-10): the missing "vdo sound" browse surface — Sound tool gains a 🎬 "Browse video sounds" row (both platforms) opening a dedicated sheet listing every kind 21/22 note in the LIVE feed window with an https media URL (opening fires one bounded REQ `bitos-video-sounds` `{"kinds":[21,22],"limit":100}` so the window fills while browsing; CLOSE on dismiss); "Use sound" → `attachSoundFromVideoUrl` (new extraction of the Wave-C seed path: bounded download → passthrough audio extract → decode → attach with provenance sourceNoteId=video event / sourceAuthorPubkey — NO url, the m4a uploads hash-verified at publish, then sound/p/attribution stamp). Android `VideoSoundsBrowseSheet` composable + editor REQ/CLOSE effect; iOS `VideoSoundsBrowseSheet` view (feedStore.notes filtered; relayPool REQ on appear / CLOSE on disappear). Answers the UX gap: the sheet previously only offered trending (derived borrows) + kind-30078 + local — no direct kind 21/22 browse.
  - [x] SOUND SHEET UX REWORK + NIP-50 SEARCH (2026-09-10): the Sound tool's pick surface is now ONE sectioned sheet on both platforms — 🔍 search field (client-side label/license filter is PRIMARY; typing ≥ 3 chars ALSO fires a bounded NIP-50 relay REQ `bitos-sounds-search` `{"kinds":[30078],"search":"…","limit":100}` with CLOSE-first, docs caveat "relay search optional — never assume universal support"), 🔥 Trending entry (opens the EXISTING §3.21 rail — TrendingSoundsScreen/TrendingSoundsView with preview+waveform — now reachable from INSIDE the editor; "Use" attaches to the CURRENT project through the new `attachSoundFromUrl`/`attachAudioFromUrl` extraction of the Wave D path), ♪ Shared sounds rows (online rail, filtered), From my library rows (filtered), Pick sound from a video. Android: editor-owned `SharedSoundStore` + `feedRepository.state` for trending detection; iOS: `editorSoundStore` + `environment.feedStore.notes` (soundUrl projection), Swift-6-safe local search field.
  - [x] MST-045 PUBLISH-YOUR-OWN TEMPLATE — THE WRITE PATH, BOTH PLATFORMS (2026-09-10): shared `NoteComposer.composeSharedTemplate` (kind-30078 `com.bitos.bitz:template:*` — slug id, 8-icon allowlist, tier-only prices {0,21,100,500} (off-tier never signs), 10 categories, local overlays convert through the ONE wire encoder so `MemeTemplateContract.parse` round-trips what this signs; `SharedTemplateComposerTest` battery incl. sticker-survives/IMAGE-drops + signed-frame pins) + bridge pair `memeSharedTemplateEventId`/`memeSharedTemplatePublishMessage` (projectJson in, pinned clock) + Android `NotePublisher.publishSharedTemplateNote` + `MediaPublishViewModel.publishSharedTemplate` (PURE DATA — no upload) + MEME panel "Publish as template…" row → dialog (label/icon/price/category) both platforms; iOS `MemeEditorStore.publishSharedTemplate` + `hasPublishableCaption` + Form dialog sheet. Lanes: 788 macosArm64 ✓, android ✓, ios-build.sh type-check ✓. Remaining from MST-045: zap unlock ledger (MST-046), icon-allowlist sync with web.
  - [x] MST-047 shared sounds WAVE 4b — NATIVE LIBRARY + PUBLISH-OWN, BOTH PLATFORMS (2026-09-10): ANDROID `SharedSoundLibraryStore` (studio/sounds/index.json + <id>.bin; shared codec via `SharedSoundLibrary.add`, ingest-gated, LRU deletes evicted artifacts) + Sound tool "From my library…" rows (offline re-attach: local bytes sha-verified → decode → attach, entry URL re-attaches without re-upload) + auto-save on every attach (dedup by sha-prefix id) + "Publish this sound…" dialog (label ≤40, HARD 3-CC license picker, description ≤500, ≤15 s guard) → `MediaPublishViewModel.publishSharedSound` (Blossom upload hash-verified → `NotePublisher.publishSharedSoundNote` → `composeSharedSound` → sign → relays; named failures; url rides state) — and the editor publish path now stamps `soundtrackProjectJson` unchanged. iOS `SharedSoundLibraryStore.swift` (Application Support/studio/sounds; bridge library trio; pbxproj registered) + MemeEditorStore `persistSoundToLibrary`/`attachLibrarySound`/`publishSharedSound` (BlossomUploader + `NotePublisher.publishSharedSoundNote` via the bridge pair; published URL refreshes the library entry) + SoundPanelContent library/publish sections (split for the type-checker) + publish sheet (Form + inline CC picker); FIX: SoundtrackRow now carries src/author through volume/loop re-applies (latent provenance loss on tweak). Lanes: 782 macosArm64 ✓, android host+unit ✓, ios-build.sh full type-check ✓.
  - [x] MST-047 shared sounds WAVE 4a — LOCAL LIBRARY + PUBLISH-OWN CONTRACTS (2026-09-10): `NoteComposer.composeSharedSound` (pure kind-30078 write path — slug id, https-only url, canonical sha, CC-license gate, ≤15 s, bounded topics/attribution/description/image; output ROUND-TRIPS through `SharedSoundContract.parse`, so rail readers see exactly what this built; built only after upload hash-verifies per the never-sign-first rule) + `SharedSoundLibrary` (versioned `{v,sounds[]}` index codec — ≤30 entries, LRU by savedAtMs, dedup by id, junk rows drop, blank-url local-only picks allowed) + bridge seams `memeSharedSoundEventId`/`memeSharedSoundPublishMessage` (pinned-clock id/signature lockstep) + `memeSharedSoundLibraryDecode/Encode/Add` + `NostrKinds.APP_DATA = 30_078`. 16 new common tests (composer battery incl. round-trip + nulls; library LRU/dedup/hostile; bridge lockstep). Remaining W4b: native library store (filesDir/studio/sounds · Application Support) + editor Sound tool "From my library…" + "Publish this sound…" (Blossom upload → compose → sign → relay) on both platforms.
  - [◐] MST-047 shared sounds WAVE 2+3 — RAIL + "USE SOUND" HANDOFF, BOTH PLATFORMS (2026-09-10): Android `SharedSoundStore` (kind-30078 verified frames → d-tag `com.bitos.bitz:sound:*` → bridge `memeSharedSoundSummary`, newest-wins ≤ 24, the kind-30078 event id rides the row for provenance) + Create hub "Shared sounds" rail (♪ hex-plate cards, license chip, duration) → tap seeds the editor through `MemeSoundSeed(isAudioOnly=true, sha256=x-tag)` — the MST-050 Wave C/D path (https-only, bounded download, sha256 verify, decode, attach; publish stamps sound/p/attribution WITHOUT re-upload; provenance = the kind-30078 event). iOS `SharedSoundStore` (@MainActor/@Observable; FrameIngest.pump + REQ `bitos-sounds`) + `sharedSoundsSection` on CreateView (TemplateCoverCard ♪ + "Ns · license" chip) + `soundSeed:` through MemeEditorView (init-order + onDismiss/onSlotsChanged reset) + client surface `memeSharedSoundSummary`/`memeSharedSoundIngestCheck` (protocol + Framework + Fixture impls); file registered in project.pbxproj. iOS validated by the ios-build.sh fallback type-check (no simulator runtime on this machine). Remaining: W4 local library persistence (≤30/≤8MB/≤15s via `ingestCheck`) + publish-your-own sound write path.
  - [◐] MST-047 shared sounds WAVE 1 — READ CONTRACT (2026-09-10): shared `SharedSoundContract` (kind-30078 `com.bitos.bitz:sound:*` parse — tags `url/x(sha256)/license/attribution ≤140/t ≤10×40/image ≤2048/p`, content `label ≤40/durationSec ≤15/mime ≤64 default audio/webm/description ≤500`; LICENSE GATE: only CC0-1.0/CC-BY-4.0/CC-BY-NC-4.0 ingest, everything else null; canonical 64-hex sha expectation; hostile-tolerant clamps, junk duration degrades to undeclared) + pure download-side gate `ingestCheck` (decoded 1–15 s, bytes ≤ 8 MB; local library ≤ 30) + bridge seams `memeSharedSoundSummary`/`memeSharedSoundIngestCheck` + fixture `contracts/meme/sound-event-v1.json` (pinned verbatim in `SharedSoundContractTest`, 11 tests, + bridge seam tests).
  ...
  - [x] Blank-video duration fixes, round 3 — full-chain audit + self-heal (2026-09-08, BOTH platforms, user-reported "set 10 s → export 0 s / preview not 10 s"): every consumer verified against the picked window — preview (VideoStage clips startMs..endMs), MP4 export + publish (`exportClips` ClippingConfiguration), video→GIF sampling, resume, undo/redo. Two real residual causes fixed: (1) CORRUPTED DRAFTS — slots saved by the pre-fix bug carry a degenerate 0..0 wire window and every relaunch restored 0 s forever; blank-prefixed clips now SELF-HEAL on restore/reconcile (`endMs ≤ startMs` + honest probe length > 0 → window = full length; next autosave persists the repair) on both platforms. (2) DEVICE-BROKEN SYNTH — `BlankClipSource.create` now VALIDATES its own output with the honest (extractor-backed) probe (≥ requested − 400 ms), retries once with a zero pts base (base-sensitive encoders), and refuses loudly ("the device encoder produced a broken clip; try again") instead of shipping a 0 s canvas. Verification: app 127/127, iOS full-target typecheck clean.
  - [x] Blank-video duration fixes, round 2 — the 0-second timeline could COME BACK after autosave/undo/relaunch (2026-09-08, BOTH platforms, user-reported "export time not match / progress time not match"): the previous fix made the CREATED window exact, but three consumers still clamped windows to `probe.durationMs` — and on devices whose retriever returns no duration metadata for the tiny no-audio synth file (the `?: 0L` fallback), that clamp re-collapsed the 3 s window to 0..0 on RESUME (`restoreVideoClipsFrom`), UNDO/REDO (`reconcileClipsFromWire`) and the publish size-ladder re-trim (which could also CRASH: `coerceIn(200, 0)` is an empty range). ROOT FIX — the probe itself is now honest: a missing/zero duration read falls back to the container's video-track header (`MediaExtractor` `KEY_DURATION`, which MediaMuxer always writes) before ever reporting 0 (`probe`/`probeFile`, Android). DEFENSE-IN-DEPTH — windows never clamp to a 0-bound probe anywhere: reconcile/resume/size-ladder (Android) and `rebuildClipsFromWire`/`restoreClips` (iOS) treat `durationMs ≤ 0` as "no bound" (Int64.max). Net effect: the picked duration is the timeline, the stage progress, the export and the persisted draft — identical everywhere. Verification: app 127/127, iOS full-target typecheck clean.
  - [x] Blank-video duration fixes (2026-09-08, BOTH platforms, user-reported "time not match / export shows 0 s / Next disabled"): ROOT CAUSE — `probeRetriever` falls back to `durationMs = 0` when a device's retriever can't read duration metadata off the tiny no-audio synthesized file, and the blank pipeline trusted that probe: `cutForDuration(0)` produced a 0..0 window → 0 s timeline (Export sheet "0 s"), and the duration mismatch on restyle. FIXES — (1) the blank clip's window is now the EXACT PICKED duration (create + re-style), never clamped to the probe — only the remaining timeline budget may trim it (honest notice); (2) re-style EXTENDS the window to the new duration (it previously kept the old short window — picking 10 s on a 3 s canvas changed nothing); (3) synthesizer presentation times now start at a 1 s base (some surface encoders mishandle a zero timestamp); (4) duration chips extend to 30 s / 60 s (the publish kind-22 cap; FULL profile allows more — synthesis cost is trivial); (5) a SOUNDTRACK now counts as blank content (`blankVideoHasContent`) — music over a canvas IS the meme — so Next/publish enable; (6) content-less blank sessions explain themselves in the status line ("Add text, stickers, layers, sound or a soundtrack to export & publish") instead of silently disabled buttons. Long-timeline behavior verified by construction: appends gate on the profile budget, video→GIF samples the first 10 s with the honest span note, publish keeps the 60 s kind-22 / longer kind-21 ladder. Verification: app 127/127, iOS full-target typecheck clean.
  - [x] Hardening — Wave 5 / close-out of `meme-blank-canvas-crossmode-plan.md` (2026-09-08): MST-085 — QA section **S7** in `docs/product/qa-manual-checklist.md`: 12-step matrix (blank IMAGE/VIDEO/GIF sessions, canvas re-style, resume per mode, kinetic overlay WYSIWYG, video→GIF conversion incl. first-10-s/ladder/double-paint guards, durable-job retry, per-mode publish) + device perf gates (synthesis ≤300 ms/≤100 KiB, sampling ≤~10 s/≤60 MiB peak, 60-frame/8 MB ladder). MST-086 — derived GIF timing SINGLE-SOURCED in shared `MemeCanvas` (`blankGifFrameCount`/`loopDelayMs` + `VIDEO_GIF_FPS/MAX_SPAN` + `videoGifFrameCount/videoGifDelayMs/videoGifSpanTrimmed`), used by the Android blank-GIF source and `VideoGifSampler`; common test `derivedGifTimingClampsIdentically` pins the clamps (iOS mirrors the values as literals — noted in the plan). MST-087 — doc sweep: `meme-studio-plan.md` cross-links the crossmode plan (MST-070..087), plan status → COMPLETE, this tracker carries one entry per wave. Verification: shared 749/749, app 127/127, iOS full-target typecheck clean (framework regenerated).
  - [x] Video→GIF export — Wave 4 of `meme-blank-canvas-crossmode-plan.md` (2026-09-08, BOTH platforms): the Export sheet's new **Format** row (video mode) offers **MP4 / GIF**; GIF exports SAMPLE the composed timeline and ride the existing GIF ladder — raw clip windows are decoded directly (Android `VideoGifSampler`: MediaMetadataRetriever `OPTION_CLOSEST` per output moment, temp files per clip, letterboxed onto the first frame's 480 px canvas; iOS `sampleVideoGifFrames`: `AVAssetImageGenerator` with ±50 ms tolerance over `EditorClip` URLs + `clipOutputMs` math) so the GIF encoder paints the TIMED overlay plan per frame — overlays, fx kinetics (Wave 3), layers, looks, trims and speed arrive exactly like the MP4 burn, with NO double-paint and NO extra compose/transcode pass. Bounds surfaced honestly: ~10 fps, up to the FIRST 10 s (longer timelines say "· first 10 s" in the outcome), auto-downscale via the ≤3-step/8 MB ladder ("Saved at a smaller size" outcome reused). Durable-job semantics preserved: the GIF-from-video rides `MemeExportJobStore` as a `gif` job — Retry reuses the artifact (MUX-05). Publishing stays MP4 in V1 (MST-084 stretch declined for now; save-only, per plan §3.4). Verification: shared 748/748, app 127/127, iOS full-target typecheck clean.
  - [x] Kinetic GIF — Wave 3 of `meme-blank-canvas-crossmode-plan.md` (2026-09-08, BOTH platforms + shared): MST-077 — GIF exports go fx-per-frame: `renderFrameRgba`/`MemeGifExportIos.encode` now paint the TIMED plan (`MemeExportRules.paintPlanAt` per step; new `memeExportPlanAt` bridge seam returns rows carrying `fxScale|fxRot|fxDx|fxDy|fxAlpha`, invisible rows dropped) — pop/fade/shake/spin + visibility windows burn into GIFs exactly like video; the picked-GIF stage preview animates at the current frame's loop time (Android `transformAt` direct; iOS via the sheet). Static envelopes keep their byte-shape (no fx keys) — old consumers unchanged. MST-078 — blank GIF sessions: creation sheet (ratio · bg · loop 1/2/3 s · fps 10/15) → canvas + fx-animated overlays on a loop clock; export/publish generate ONE solid canvas bitmap repeated at loop timing (the timed plan paints per frame) through the existing GIF ladder; §3.5 content gating (overlays/strokes required — "the canvas alone is not a GIF"). MST-079 — wire: additive `canvas.sec`/`canvas.fps` (clamped shared-side 500 ms–10 s, fps ∈ {10,15}; junk drops, old wires byte-identical) + `MemeSetCanvasTiming` state method + `memeSetCanvasTiming` bridge seam; common tests: timing round-trip/clamps/additivity + timed-envelope fx rows. MST-080 — resume: blank GIF infers from mode+canvas (timing rides the wire → relaunch resumes the loop); the per-mode Speed chip becomes **Duration** on a blank session (re-opens the sheet as re-style). iOS note: the sheet chain was split (`sheetedLayer`/`interactionLayer`) after the new sheets pushed SwiftUI's expression checker past its budget. Verification: shared 748/748, app 127/127, iOS full-target typecheck clean (framework regenerated with the new seams).
  - [x] Blank VIDEO Wave 2 — sound + layers QA + resume contract (2026-09-08, BOTH platforms): MST-074 — the audio bed is DURATION-driven (`audioBedTimeline` → shared `SfxSynth.renderCueTrack(cues, timelineMs)` + soundtrack `MemeSoundMix.bedTrack`), so blank timelines mix cues/soundtracks by construction; the shared math was already common-tested (offsets, out-of-range drops, rate scaling in `SfxSynthTest`); fixed the dishonest copy — the sound panel said "Original clip audio" for a source with NO audio track; blank sessions now read "Silent canvas / The blank canvas has no audio of its own — add SFX or a soundtrack" (`SoundPanelContent(hasSourceAudio:)`, both platforms). MST-075 — layers on blank verified: image/animated-GIF inserts are mode-agnostic overlays over any clip (stage + export seams fixed in the image-stack wave); `restyleBlankClip` preserves asset ids so reels and image maps survive regeneration. MST-076 — resume contract: `blankCanvasClipSurvivesSaveAndRelaunch` proves the reserved `blank1` id decodes like any clip (shared decode treats ids as opaque), the slot join restores the synthesized bytes, and the §3.5 gating inputs (overlays) ride the same wire. Verification: Android 27/27 meme tests incl. the new one; iOS full-target typecheck clean.
  - [x] Blank VIDEO — Wave 1 of `meme-blank-canvas-crossmode-plan.md` (2026-09-08, BOTH platforms): "the canvas IS the media." MST-070 synthesizer — a solid-color, silent, exact-duration H.264 MP4 generated in milliseconds via explicit presentation times (Android `BlankClipSource`: EGL+MediaCodec/MediaMuxer, `eglPresentationTimeANDROID`; iOS `BlankClipSourceIos`: AVAssetWriter+pixel-buffer adaptor), probe-able, a few dozen KiB, 720 px long edge, 30 fps. MST-071 creation — video-mode empty state gains "Start blank canvas" → the New-blank sheet (ratio 1:1/4:5/9:16/16:9 · CanvasSheet background swatches · duration 3/5/10 s) → `appendClip(…, id = "blank1")`: the reserved `blank` id prefix is the wire-visible session marker, and the clip rides the FULL M5 machinery (timeline, trim/split/speed/volume/look, SFX mix, animated layers, export, publish, slots) unchanged. MST-072 detection + gating — `blankVideoActive` (all sources blank) → timeline segment labels "canvas"; §3.5 honesty: blank sessions need ≥1 overlay/stroke/cue to export or publish (header Next, Export chip, save + publish guards on both platforms). MST-073 re-style/extend — the Canvas chip appears in the video bar on a blank timeline and re-styles THE SOURCE: one sheet (ratio · bg · duration) regenerates the clip in place; overlays/strokes/cues keep timeline positions (shorter durations clamp windows). Deviation from plan: Extend lives in the Canvas sheet (one home for all canvas facts) instead of the clip sheet; canvas spec is session-only prefill (ratio/bg ride the clip bytes). Verification: Android compile + 126/126 unit tests; iOS full-target typecheck (regenerated the stale BusinessCore device framework first — the tree's bridge had moved past the old prebuilt).
  - [x] IMAGE-STACK FIX — layers were invisible + dropped from exports outside video mode (2026-09-08, BOTH platforms, user-reported "insert image should stack to canvas — not working"): the stack SEMANTICS were right (first pick = background, later picks = draggable IMAGE overlays via `addImageOverlay`; Layers-sheet insert too), but every image/GIF-mode render path never received the layer bitmaps — only VideoStage did. Fixed all six seams: Android stage (GIF + image branches now pass `imageAssets = imageAssetMap` to `OverlayNode`), Android PNG/GIF export (`MemeRaster.render`/`renderBlank`/`renderFrameRgba` take `imageFor`; `MemeGifExport.export` threads it; `decodeLayerBitmaps()` decodes each layer ONCE per export at the bounded sample), iOS stage (both branches pass `layerImage: overlay.assetId.flatMap { store.layerImages[$0] }`), iOS PNG export (`renderPngData`/`renderBlankPngData` take `images:` → `paint(row:images:)`), iOS GIF export (`MemeGifExportIos.export/encode` take `images:`). Publish + Make-Variations paths updated on both platforms, so stacked layers burn into saved PNGs, exported GIFs, published media and variation posters identically to the video pipeline.
  - [x] Blank-canvas creation + GIF browsing (2026-09-08, BOTH platforms): (1) IMAGE mode can now START FROM SCRATCH — "Start blank canvas" on the empty state pins a 1:1 white canvas (the Canvas chip re-styles ratio/background anytime; `setCanvas` was already undoable). A blank design = IMAGE mode + no picks + pinned ratio; the stage keeps the full overlay/gesture surface over the background fill, and every downstream path is blank-aware: PNG export (`MemeRaster.renderBlank` Android / `renderBlankPngData` iOS — same 1080-px long-edge + even-dim + shared export-plan rules), Make Variations (designEligible + render), publish (mediaReady + render + imeta dims = the rendered canvas), Export sheet dims/format rows, and header Next / Export chip enablement. Overlay coordinates are normalized so nothing else moves. (2) GIF mode can now BROWSE GIFs — a "GIFs" chip in the per-mode bar + "Browse GIFs" on the empty state open the SAME shared composer GifPickerSheet (trending cache, recents, search; GIFs tab, not stickers); picks download bounded (≤24 MiB / `GifDecoder.MAX_INPUT_BYTES`) and decode straight into the frame tray through the SAME import path as photo picks (Android `importGifBytes` extracted for both; iOS `handleGifBrowsePick` → `addGifFrames`), capped at 60 frames with a notice at the cap. Still picks land as one 100 ms frame, same as before.
  - [x] Selection action rail — MUX-03 relayout (2026-09-08 rev 2, BOTH platforms): per-element actions moved from the inserted bottom row (10 buttons squeezed under 48 dp targets on ≤410 dp screens, and every selection toggle shifted the layout) to a narrow vertical pill (52 dp/pt, 44 dp/pt targets, scrim + border, RTL mirrors) that is a LAYOUT SIBLING of the canvas — not an overlay — so the media is never covered: selecting smoothly reserves an animated trailing gutter (expandHorizontally/.animation on selectedId) and the canvas rescales into it; overlay coordinates are normalized to the fitted stage, so geometry, gestures and export are untouched. Precision fine-tuning (nudge pad ±0.05 · shrink/enlarge ×0.9/×1.1 · rotate ±15°) moved OUT of the canvas entirely into the app's standard tool sheet (rail ▸ Move opens it; `PrecisionSheetContent`/`precisionControlsSheet`, D-pad cluster + zoom + rotate rows + Done) — stacking a panel beside the rail would have eaten another ~96 dp of canvas on phones. The rail recedes to 35% alpha while a direct gesture is in flight (Android `MemeEditorState.gesture` now observable state; iOS store `gestureActive`). Rail actions: edit text · duplicate · bring forward · send backward · move ▸ · delete. NEW actions: duplicate clones an overlay verbatim (style, caps/bar, fx, visibility windows, asset binding) with a fresh deterministic id and a +0.05 diagonal offset — Android `MemeEditorState.duplicateOverlay` (typed `AddOverlay`, `duplicateClonesStyleWithAFreshIdAndOneUndoStep` test), iOS `MemeEditorStore.duplicateOverlay` (raw-wire clone so optional fields survive; shared decode+apply re-clamps). Z-order rides the existing `reorder` command. ICON FIX: rotate buttons borrowed `Repost` (repeat arrows = "share again") and `Refresh` (circular reload) — now dedicated `AppIcons.RotateLeft`/`RotateRight` (material rotate glyphs); iOS already used `arrow.counterclockwise`/`arrow.clockwise`. Suite mode keeps its own affordances (rail is basic-editor only).
  ...
  - [x] Recent-hashtags reuse — shared ledger + chip rows (2026-09-06, BOTH platforms): new shared `publish/RecentHashtags` (normalize to the NIP-22 hashtag shape, recency merge with bump-to-front, ≤ 64 entries, `{"v":1,…}` JSON with junk-tolerant decode, `hashtagsIn` caption scan; 5 common tests) + bridge seams `recentHashtagsMerge`/`recentHashtagsSuggest` for iOS. Persisted natively (iOS `RecentHashtagsStore` in BusinessCoreClient.swift over UserDefaults via the bridge rule; Android `data/publish/RecentHashtagsStore` over SharedPreferences calling the shared rule directly — singleton, versioned key). Recorded when a publish is INITIATED (note composer, poll composer, meme flow — queued/offline publishes still count). Chip rows: iOS composer (`recentHashtagChips` under the editor, appends `#tag ` to the text) + meme details tags card (tap = addTag); Android CreateNoteScreen (caret-at-end append via TextFieldValue) + meme details tags Column (`recentHashtagChips` composable). Chips exclude what the current post already carries. Side fix: Stories `StoryVideoLayer.Coordinator` (concurrently-added file) needed `: @unchecked Sendable` for its main-queue observers to pass the Swift 6 gate. Gates: shared host tests + Android 116 tests + iOS swift-6 strict all green.
  - [x] Preflight layout polish (2026-09-06, BOTH platforms): the caption preview moved directly under the "imeta dims + sha256 pinned at upload…" facts line (inside the thumb row's text column, top-aligned) instead of a separate section below the row; and the header "Cancel" button was REMOVED — the back arrow already closes the flow at the details step, so the pair conflicted. Gates: Android 116 tests + iOS swift-6 strict green.
  - [x] Preflight caption preview (2026-09-06, BOTH platforms): the preflight summary now renders the real caption with hashtag/mention highlighting via the SAME shared NIP-27 pipeline the feed cards use — iOS `ExpandableRichText` fed by the bridge `richTokens(content:)` factory (inert callbacks, re-tokenized `.task(id: caption)` when the user edits and returns), Android `RichText(tokens = Nip27.tokenize(caption))` under the thumb row (tokens memoized per caption). Empty caption stays hidden (the checklist already reports "(none)"). Gates: Android 116 tests + iOS swift-6 strict green.
...
- 2026-09-10 — **Browse video sounds (kind 21/22) in the Sound
  sheet** (left uncommitted for owner review): the "vdo sound" browse
  gap closed — 🎬 row → sheet over the live kind-21/22 window (bounded
  REQ while open, CLOSE on dismiss); "Use sound" rides the extracted
  Wave-C path (`attachSoundFromVideoUrl`: download → extract → attach
  with provenance, uploads at publish). Both platforms; all lanes ✓.

- 2026-09-10 — **Sound sheet UX rework + NIP-50 search** (left
  uncommitted for owner review): one sectioned pick surface (search /
  trending / shared online / local library / pick-from-video) on both
  platforms; the §3.21 trending rail is now reachable from inside the
  editor and attaches to the current project (Wave-D hash-verified URL
  path, extracted into `attachSoundFromUrl`/`attachAudioFromUrl`).
  Search: client-side filter primary + optional NIP-50 REQ (CLOSE-first,
  docs caveat: relay search support is optional). 792 shared tests ✓,
  android ✓, ios-build.sh type-check ✓.

- 2026-09-10 — **MST-045 publish-your-own template — the write path,
  both platforms** (left uncommitted for owner review): pure-data
  kind-30078 template publish (no upload) — shared
  `composeSharedTemplate` (round-trips the rail parse; tier-only
  prices, 8-icon allowlist), bridge pair, NotePublisher methods,
  view-model flow + MEME panel row + label/icon/price/category dialog
  on Android and iOS. 788 common tests (6 new).

- 2026-09-10 — **MST-047 shared sounds WAVE 4b — native library +
  publish-your-own, both platforms** (left uncommitted for owner
  review): local library stores (index via the shared codec + one .bin
  per sound, ingest-gated, LRU artifact cleanup), Sound tool "From my
  library…" offline re-attach (sha-verified local bytes), auto-save on
  attach, and "Publish this sound…" (CC-only license picker →
  hash-verified Blossom upload → `composeSharedSound` → sign → relay —
  never sign before upload). iOS also fixes a latent provenance loss
  (src/author now survive volume/loop tweaks). The MST-047 wave table
  is complete: contract → rail → handoff → library + publish-own.

- 2026-09-10 — **MST-047 shared sounds WAVE 4a — the library +
  publish-own contracts**: `composeSharedSound` (kind-30078 write path,
  round-trips the read contract) + `SharedSoundLibrary` (LRU index
  codec) + bridge seams (compose pair + library trio) +
  `NostrKinds.APP_DATA`. 782 common tests green (16 new). W4b native
  store + Sound tool rows next.

- 2026-09-10 — **MST-047 shared sounds WAVE 2+3 — rail + "Use sound"
  handoff, both platforms**: Android `SharedSoundStore` + Create-hub
  "Shared sounds" rail + `MemeSoundSeed(isAudioOnly)` handoff; iOS
  `SharedSoundStore` + `sharedSoundsSection` + `soundSeed:` through
  MemeEditorView + client surface (`memeSharedSoundSummary`/
  `memeSharedSoundIngestCheck` × protocol/Framework/Fixture) + pbxproj
  registration. Reuses the MST-050 Wave C/D attach machine verbatim
  (hash-verified, no re-upload, provenance = the kind-30078 event).
  Lanes: macosArm64 766✓, testAndroidHost + testDebugUnitTest ✓,
  ios-build.sh fallback type-check ✓ (no simulator runtime here).
  Remaining: W4 local library + publish-your-own sound.

- 2026-09-10 — **MST-047 shared sounds WAVE 1 — the read contract**
  (the kind-30078 licensed library; APP-021's named dependency): shared
  `SharedSoundContract` + `SharedSoundContractTest` (11 battery) + bridge
  seams `memeSharedSoundSummary`/`memeSharedSoundIngestCheck` + fixture
  `contracts/meme/sound-event-v1.json`. License-gated ingest (CC0\/BY\/BY-NC
  only — stricter than remix's advisory gate), canonical sha256
  expectation, `ingestCheck` for the download side (≤15 s, ≤8 MB),
  hostile-tolerant bounds per plan §3.5. All 766 common tests green.
  Next: W2 relay rail (`SharedSoundStore` both platforms), W3 "Use
  sound" handoff via the MST-050 Wave C seed path, W4 local library +
  publish-your-own.
...
- 2026-09-08 (flake investigation — dossier; NO code change) —
  **FeedRepositoryTest class flake** (owner: user; expiry: next release
  ...
  at 20 s): `buildsFollowingTimelineFromContactList` (2/3 captures) and
  `hydratesFollowingListFromCachedContactHead` (1/3). Repro: the FULL
  class, repeated — 3/20, 2/20, 2/20 across batches (≈7–15%); SOLO
  0/25+0/15; the exact predecessor subset 0/10; pair 0/12 → LOAD/timing-
  dependent, not order/state contamination (fresh instances per test;
  the default projection store is per-instance in-memory). Eliminated by
  audit+probes: frame-loss windows (transport replay 8 → pool always-on
  collectors replay 32 → repository collector — every hop replay-safe,
  no connection gating); accountPubkey visibility (emit-chain
  happens-before); absorb determinism (fixed fixtures, newest-head picks
  the only candidate); state visibility (`state` IS the mutated
  StateFlow); PerfTrace (JVM no-op); FollowingProjectionStore (per-
  instance). One captured failure showed NO frame/absorb probe lines for
  the victim — but cross-thread stdout capture ordering makes that
  inconclusive. NEXT: instrument with a per-test coroutine-name +
  monotonic-nano prefix and capture via a dedicated TestWatcher listener
  (not println), or run the class under a deterministic single-threaded
  test dispatcher to force the interleave; also consider replacing the
  delay(10) polling with `withTimeout + awaitFrame` spin on the state
  flow itself. Suspected area: scheduling interplay between the
  runBlocking event loop and Dispatchers.Default collectors under class-
  length load. NOT retry-masked (flake policy).

- 2026-09-08 (sound follow-ons; left UNCOMMITTED for owner review) —
  **MST-050 polish: soundtrack LOOPING + per-overlay cue anchors.**
  Shared (common-tested): `MemeSoundtrack.loop` wire flag (row + command
  codec round-trip) and `MemeSoundMix.bedTrack(loop=…)` — the placed
  track repeats cyclically to fill the timeline (gain kept, degenerate
  shapes safe); both bed seams (Android direct + bridge
  `memeAudioBedWavBase64`) pass it through. Both platforms: Sound panel
  "Loop to fill" toggle (state op resend / command re-apply); the
  preview players wrap the position modulo the AUDIBLE window
  (start-in-point aware) so repeats stay glued to the clock; the SFX
  sheet gained the per-overlay cue anchor — a selected timed overlay
  offers "At overlay start / At overlay end" and ＋ schedules there
  instead of the raw playhead. One FLAKE recorded: FeedRepositoryTest
  `hydratesFollowingListFromCachedContactHead` failed once mid-run then
  passed with AND without this change (per the flake policy it needs an
  owner + investigation — not touched here). Verified: full shared +
  Android suites green; Swift 6 typecheck 0 errors.
